...
async function performSyncOneSeriesFromLiveChart(store, series) {
...
const activeSeriesSyncs = new Map();

export async function syncOneSeriesFromLiveChart(store, series) {
  const key = series?.id || series?.scheduleLink;
  if (!key) return performSyncOneSeriesFromLiveChart(store, series);
  if (activeSeriesSyncs.has(key)) return activeSeriesSyncs.get(key);

  const activeSync = performSyncOneSeriesFromLiveChart(store, series);
  activeSeriesSyncs.set(key, activeSync);
  try {
    return await activeSync;
  } finally {
    activeSeriesSyncs.delete(key);
  }
}

let activeLiveChartSync = null;

async function performSyncAllLiveChart(store, options = {}) {
...
export async function syncAllLiveChart(store, options = {}) {
  if (activeLiveChartSync) return activeLiveChartSync;

  activeLiveChartSync = performSyncAllLiveChart(store, options);
  try {
    return await activeLiveChartSync;
  } finally {
    activeLiveChartSync = null;
  }
}
